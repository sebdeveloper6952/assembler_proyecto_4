...
* C++ Signature: void* GetGpioAddress()
  input: r0 = raspberry pi version, 1, 2, or 3
*/
...
GetGpioAddress:
	...
	push {lr}
        cmp r0, #1
        ldreq gpioAddr, =0x20200000
        cmp r0, #2
	ldreq gpioAddr,=0x3F200000 @GPIO base para raspberry 2
        cmp r0, #3
        ldreq gpioAddr, =0x3F200000 @GPIO base para raspberry 3
        ldr r1, =baseAddr
        str r0, [r1]                  @guardar direccion base
	bl phys_to_virt
...
/* GetGpio gets the logic level from specified pin number. 1 = HIGH, 0 = LOW
   input:  r0 -> pin number
   output: r0 -> 1 (HIGH) || 0 (LOW) || -1 ERROR */
.global GetGpio
GetGpio:
  push {lr}
  ...
  cmp r0, #53
  movgt r0, #-1
  movgt pc, lr                                  @salir de subrutina por error
  cmp r0, #1
  movlt r0, #-1
  movlt pc, lr                                  @salir de subrutina por error

  mov r1, r0                                    @guardar pinNum en r1
  ldr r0, =baseAddr
  ldr r0, [r0]                                  @direccion base de gpio
  ldr r0, [r0, #0x34]                           @direccion de registro de lectura
  
  mov r2, #1
  lsl r2, r1                                    @lsl r2, pinNum
  and r0, r2
